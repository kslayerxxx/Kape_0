...
(function () {
  ...
  const RH_HOST_OK = /(^|\.)runninghub\.ai$/i.test(location.hostname) ||
  ...
  if (IN_IFRAME) {
    try { PARENT_REACHABLE = !!(window.top && window.top.document && window.top.document.body); }
    catch (_) { PARENT_REACHABLE = false; }
  }
  if (IN_IFRAME && PARENT_REACHABLE) {
    console.info("[RH Ext] iframe copy standing down — the top frame owns the engine");
    return;
  }
  ...
  const onEditorHotkey = (e) => {
    ...
    if (e.ctrlKey && e.shiftKey && e.code === "KeyE") {
      e.preventDefault();
      try { openScriptEditor(); } catch (err) { console.error(TAG, "editor failed", err); }
    }
  };
  ...
  function expandTaskPanel() {
    try {
      ...
      const btn = wrap.querySelector(".hide-btn") || d.querySelector(".hide-btn");
  ...
  function findCancelButton() {
    ...
    for (const d of docs) {
      const btn = d.querySelector(".rh-task-item .rh-cancel-btn") ||
    ...
    for (const d of docs) {
      let all;
      try { all = d.querySelectorAll("button, div, span, a, [role='button']"); } catch (_) { continue; }
      for (const el of all) {
  ...
  function triggerRHInterrupt(t) {
    ...
    let attempts = 0, announced = false, confirmed = false;
    ...
    const watch = setInterval(function () {
      ...
      try {
        const d = topDoc();
        const root = d.querySelector("#rh-message-root") || d.querySelector(".ant-message") ||
        ...
        try {
          ...
          if (!announced) {
            ...
            rhToast("🛑 Cancel clicked (" + hit.how + "): " +
              ((hit.el.textContent || "").trim() || "rh-cancel-btn").slice(0, 24));
          }
  ...
  function frameHasLiveCanvas(w) {
    try {
      return !!(w && w.LGraphCanvas && w.LGraphCanvas.prototype && w.document && w.document.body);
    } catch (_) { return false; } // cross-origin access throws
  }
  ...
  function resolveEngineTarget() {
    ...
    try {
      ...
      for (const f of frames) {
        let w = null;
        try { w = f.contentWindow; } catch (_) { continue; }
        if (!frameHasLiveCanvas(w)) continue;
        const d = (function () { try { return f.contentDocument || w.document; } catch (_) { return null; } })();
        if (d && d.body) { engineTarget = { win: w, doc: d, host: f }; return engineTarget; }
      }
    } catch (_) {}
  ...
  function enforceInstanceSettings(cv) {
    try {
      if (config.darkerGrid) {
        if (typeof cv.clear_background_color === "string" && cv.clear_background_color !== CANVAS_BG) {
      ...
      if (config.enhancedWires) {
        ...
        if (cv.links_render_mode !== undefined && cv.links_render_mode !== SPLINE_LINK) cv.links_render_mode = SPLINE_LINK;
        if (cv.link_render_mode !== undefined && cv.link_render_mode !== SPLINE_LINK) cv.link_render_mode = SPLINE_LINK;
  ...
  function redraw(cv) {
    try { cv.setDirty(true, true); } catch (_) {}
  }
  ...
  function attachEngine(t) {
    ...
    if (win.api && win.__rh_hooked_api !== win.api) {
      ...
      try {
        api.addEventListener("executing", (e) => {
          try {
            ...
            const rawId = (d && typeof d === "object")
              ...
              : d;
            if (rawId == null) { win.__rh_running = null; return; } // queue end

            win.__rh_running = { num: Number(rawId), str: String(rawId) };

            if (config.autoCenterRunningNode) {
              try {
                const cv = getLgc(t), g = getGraph(t);
                if (cv && g && typeof cv.centerOnNode === "function") {
                  const node = g.getNodeById(Number(rawId)) || g.getNodeById(rawId);
        ...
        try { api.addEventListener("execution_success", onEnd("✅ Done")); } catch (_) {}
        try { api.addEventListener("execution_error", onEnd("❌ Error")); } catch (_) {}
        try { api.addEventListener("execution_interrupted", onEnd("⏸ Cancelled")); } catch (_) {}
  ...
  function syncOverlaySize() {
    ...
    const w = dv.innerWidth, h = dv.innerHeight;
    const dpr = Math.max(1, dv.devicePixelRatio || 1);
    if (overlayState.w !== w || overlayState.h !== h || overlayState.dpr !== dpr) {
      overlayState.w = w; overlayState.h = h; overlayState.dpr = dpr;
  ...
  function nodeScreenRect(lgc, node) {
    try {
      ...
      if (conv) {
        ...
        cx = p[0]; cy = p[1];            // copy immediately: builds reuse the array
      } else {
      ...
      let titleH = 30;
      try {
        const LG = engineTarget && engineTarget.win && engineTarget.win.LiteGraph;
  ...
  function culled(r) {
    return !!(r && r.x < overlayState.w && r.y < overlayState.h &&
              r.x + r.w > 0 && r.y + r.h > 0);
  }
  ...
  function titleTick() {
    try {
      ...
      if (win.__rh_run_active && win.__rh_run_start) {
        document.title = base + " · ▶ " + fmtDur(Date.now() - win.__rh_run_start) + " · Running";
        win.__rh_title_released = false;
  ...
  function updateTaskTimerHUD() {
    ...
    try {
      ...
      if (running) {
        const own = ew.__rh_run_start ? fmtDur(Date.now() - ew.__rh_run_start) : "…";
        let label = own;
        const d = topDoc();
        const timeEl =
      ...
      if (ew && ew.__rh_last_done_str && ew.__rh_run_end &&
          Date.now() - ew.__rh_run_end < 8000) {
        if (disp && disp.textContent !== ew.__rh_last_done_str) disp.textContent = ew.__rh_last_done_str;
        if (hud.style.display !== "flex") hud.style.display = "flex";
        return;
      }
  ...
  function createFloatingMenu() {
    ...
    if (liveSrcEl) {
      ...
      liveSrcEl.textContent = "live script copy: " +
    ...
    editScriptBtn.onclick = () => { panel.style.display = "none"; openScriptEditor(); };
  ...
  function openScriptEditor() {
    ...
    let overrideSrc = null, cacheSrc = null;
    try { overrideSrc = localStorage.getItem(SCRIPT_OVERRIDE_KEY); } catch (_) {}
    try { cacheSrc = localStorage.getItem(SCRIPT_CACHE_KEY); } catch (_) {}

    const src = window.__RH_LIVE_SOURCE__;
    const label =
      src === "override" || overrideSrc != null ? "OVERRIDE (your edited copy is live)"
      : src === "cache" ? "LAST-GOOD CACHE (saving creates an override)"
      : "BUNDLED v" + VERSION + " (compiled into the app — saving creates an override)";
    const prefill = overrideSrc != null ? overrideSrc
                  : (cacheSrc != null ? cacheSrc
    ...
    ta.addEventListener("keydown", (e) => { if (e.key === "Escape") ov.remove(); });
    ...
    ov.querySelector("#rh-edit-save").onclick = () => {
      ...
      try { new Function(code); } // syntax check BEFORE saving
      catch (err) { rhToast("❌ Syntax error: " + err.message, 6000); return; }
      ...
      if (code.indexOf("__RH_SCRIPT_BOOT_OK__") === -1) {
        rhToast("❌ Not a full custom_r.js (no boot handshake) — not saved", 7000);
        return;
      }
      try { localStorage.setItem(SCRIPT_OVERRIDE_KEY, code); }
      catch (err) { rhToast("❌ Save failed: " + err.message); return; }
  ...
  // 14. Lifecycle Poller
  // ==========================================
  let engineReady = false, engineMisses = 0;
  function pollTick() {
    try {
      if (SHOULD_LOCK_TITLE) {
    ...
    try {
      ...
      if (t && t.win.LGraphCanvas && t.win.LGraphCanvas.prototype) {
        ...
        if (cv) {
          ...
          if (!engineReady) {
            engineReady = true;
            console.info(TAG, "engine ready", { host: t.host ? "iframe" : "self", scale: cv.ds && cv.ds.scale });
          }
        }
        ...
        if (engineMisses === 15) {
          rhToast("⚠️ ComfyUI engine not found — canvas features are off (frames:" +
            document.querySelectorAll("iframe").length + ")", 7000);
        }
      }
  ...
  window.__RH_EXT_TEARDOWN__ = function () {
    ...
    ["rh-floating-root", "rh-timer-hud", "rh-toast-host", "rh-modal-overlay",
     "rh-floating-style", "rh-canvas-perf-style", "rh-edit-overlay",
     "rh-firstrun-overlay"].forEach((id) => {
  ...
  window.__RH_DEBUG__ = {
    ...
    cancel: () => triggerRHInterrupt(engineTarget || { win: window, doc: document }),
    teardown: () => window.__RH_EXT_TEARDOWN__ && window.__RH_EXT_TEARDOWN__()
  };
  ...
  console.info(TAG, "v" + VERSION + " initialized", {
    IS_POPUP, IN_IFRAME, IS_WORKFLOW_PAGE, SHOW_FLOATING_UI,
    engine: engineTarget ? (engineTarget.host ? "iframe" : "self") : "pending",
    source: window.__RH_LIVE_SOURCE__
  });

  } catch (fatalErr) {
    window.__RH_EXT_INITIALIZED__ = false;
    console.error(TAG, "FATAL init error — last-good cache was NOT overwritten.", fatalErr);
  }
})();
