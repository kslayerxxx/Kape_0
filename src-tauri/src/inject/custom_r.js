...
(function () {
  ...
  if (window.__RH_EXT_INITIALIZED__ === true || window.__RH_LIVE_SOURCE__) {
    ...
    try { delete window.__RH_LIVE_SOURCE__; } catch (e) { window.__RH_LIVE_SOURCE__ = null; }
  }
  window.__RH_SCRIPT_BOOT_OK__ = true;     // handshake: we started

  function hostReady() {
    var h = location.hostname || "";
    return /(^|\.)runninghub\.ai$/i.test(h) || h === "localhost" || h === "127.0.0.1";
  }
  ...
  function start() {
    try { main(); }
      ...
      try { console.error("[RH] FATAL init error:", err); } catch (_) {}
      try { fatalToast("RH script failed to start: " + (err && err.message ? err.message : err)); } catch (_) {}
    }
  }
  ...
  function log() {
    if (!DBG) return;
    try { console.info.apply(console, [TAG].concat([].slice.call(arguments))); } catch (_) {}
  }
  function warn() {
    if (!DBG) return;
  ...
  function topDoc() {
    try { if (window.top && window.top.document && window.top.document.body) return window.top.document; }
    catch (_) {}
    return document;
  }
  function isTopDocument() {
    try { return document === window.top.document; } catch (_) { return false; }
  }
  ...
  var IS_WORKFLOW_PAGE = /\/workflow\/\d+/.test(location.pathname) || IS_COMFY_DOC;
  ...
  function nameFromUrl(u) {
    ...
    } catch (_) { return "rh-" + Date.now(); }
  }
  function absUrl(u, win) {
    try { return new URL(u, (win && win.location ? win.location.href : location.href)).href; }
    catch (_) { return u; }
  }
  ...
  function rhToast(msg, ms) {
    try {
      ...
      if (!host || !host.isConnected) {
        ...
        host.style.cssText = "position:fixed;bottom:70px;left:50%;transform:translateX(-50%);" +
      ...
      el.textContent = msg;
      el.style.cssText = "background:rgba(18,18,22,0.96);border:1px solid rgba(0,255,102,0.5);" +
  ...
  function openPasteModal(title, placeholder, onSubmit, prefill) {
    try {
      ...
      ov.style.cssText = "position:fixed;inset:0;z-index:1000001;background:rgba(0,0,0,.55);" +
  ...
  try {
    ...
    if (Array.isArray(rawScripts)) {
      customScripts = rawScripts.filter(function (s) {
        return s && s.code && s.code.indexOf("RH_QOL_CONFIG") === -1 && s.code.indexOf("createFloatingMenu") === -1;
      });
      if (rawScripts.length !== customScripts.length) lsSet(CUSTOM_SCRIPTS_KEY, JSON.stringify(customScripts));
    }
  } catch (e) { customScripts = []; }
  function saveCustomScripts() { lsSet(CUSTOM_SCRIPTS_KEY, JSON.stringify(customScripts)); }
  customScripts.forEach(function (s) {
    if (s.enabled && s.code && s.code.trim()) {
      try { new Function(s.code)(); }
      catch (e) { rhToast('⚠️ UserScript "' + s.name + '" failed'); }
    }
  });
  ...
  window.importCookiesFromJSON = function (jsonInput) {
    try {
      var cookies = typeof jsonInput === "string" ? JSON.parse(jsonInput) : jsonInput;
      if (!Array.isArray(cookies)) { rhToast("❌ Expected a JSON array of cookies"); return false; }
      cookies.forEach(function (c) {
        if (!c.name || c.value === undefined) return;
        var str = encodeURIComponent(String(c.name).trim()) + "=" +
                  encodeURIComponent(String(c.value).trim()) +
                  "; path=/; max-age=31536000; SameSite=Lax";
        if (location.hostname.indexOf("runninghub") !== -1) str += "; domain=.runninghub.ai";
        document.cookie = str;
        var k = String(c.name).toLowerCase();
        if (k.indexOf("token") !== -1 || k.indexOf("auth") !== -1 || k.indexOf("session") !== -1) {
          try { localStorage.setItem(c.name, c.value); sessionStorage.setItem(c.name, c.value); } catch (_) {}
        }
      });
      rhToast("✅ Imported " + cookies.length + " cookie(s) — reloading…");
      setTimeout(function () { location.href = "https://www.runninghub.ai/"; }, 600);
      return true;
    } catch (err) {
  ...
  function pasteAndImportCookies() {
    var go = function (text) {
      if (text && text.trim().charAt(0) === "[") window.importCookiesFromJSON(text);
      else openPasteModal("Paste Cookie JSON", '[{"name":"...","value":"..."}]', window.importCookiesFromJSON);
    };
    try {
      if (navigator.clipboard && navigator.clipboard.readText) {
        navigator.clipboard.readText().then(go).catch(function () { go(""); });
        return;
      }
    } catch (_) {}
    go("");
  ...
  function onKey(e) {
    ...
    if (e.code === "KeyE") { e.preventDefault(); try { openScriptEditor(); } catch (err) { warn(err); } }
    else if (e.code === "KeyV") { e.preventDefault(); pasteAndImportCookies(); }
  }
  ...
  if (SHOULD_LOCK_TITLE) {
    try {
      ...
      Object.defineProperty(document, "title", {
        configurable: true, get: function () { return "RunningHub"; }, set: function () {}
      });
    } catch (e) { warn("title lock failed", e); }
  }
      ...
      w.open = function (url, target, features) {
        try {
          if (isWorkflowUrl(url)) {
            if (config.sameWindowWorkflows) { location.href = absUrl(url, w); return null; }
            ...
            return orig(url, name, "width=1400,height=900,resizable=yes,scrollbars=yes");
          }
        } catch (e) { warn("open patch", e); }
        return orig(url, target, features);
      };
    } catch (e) { warn("cannot patch window.open", e); }
  }
  patchWindowOpen(window);

  /* ======================= 5. third-party telemetry only ============== */
  if (config.blockTelemetry) {
    var blocked = ["google-analytics.com", "googletagmanager.com", "hm.baidu.com",
                   "clarity.ms", "sentry.io", "analytics.google.com"];
    ...
    window.fetch = function (input, init) {
      try {
        ...
        if (u && u.indexOf("runninghub.ai") === -1 && blocked.some(function (h) { return u.indexOf(h) !== -1; })) {
  ...
  function toggleStopId(id) {
    var k = String(id);
    if (stopNodeIds.has(k)) { stopNodeIds.delete(k); return false; }
    stopNodeIds.add(k); return true;
  }
  ...
  function expandTaskPanel() {
    try {
      ...
      if (!wrap || !/(^|\s)hide(\s|$)/.test(wrap.className || "")) return false;
      var btn = wrap.querySelector(".hide-btn") || d.querySelector(".hide-btn");
  ...
  function findCancelButton() {
    ...
    if (engineTarget && engineTarget.doc && docs.indexOf(engineTarget.doc) === -1) docs.push(engineTarget.doc);
    for (var i = 0; i < docs.length; i++) {
      var d = docs[i];
      var btn = d.querySelector(".rh-task-item .rh-cancel-btn") ||
    ...
    for (var j = 0; j < docs.length; j++) {
      var all;
      try { all = docs[j].querySelectorAll("button, div, span, a, [role='button']"); } catch (_) { continue; }
      for (var k = 0; k < all.length; k++) {
        var el = all[k], txt = (el.textContent || "").trim();
        if (!txt || txt.length > 12 || !/^(cancel|取消)$/i.test(txt) || !isVisible(el)) continue;
  ...
  function triggerRHInterrupt(t) {
    ...
    var attempts = 0, announced = false, confirmed = false, MAX = 8;
    ...
    var watch = setInterval(function () {
      ...
      try {
        var d = topDoc();
        var root = d.querySelector("#rh-message-root") || d.querySelector(".ant-message") ||
                   d.querySelector(".ant-notification");
        if (root && successRX.test(root.textContent || "")) {
          confirmed = true; clearInterval(watch);
    ...
    (function tick() {
      ...
      try { if (iWin.api && typeof iWin.api.interrupt === "function") iWin.api.interrupt(); } catch (e) {}
      if (hit) {
        try {
          hit.el.click();
          if (!announced) {
  ...
  function frameHasLiveCanvas(w) {
    try { return !!(w && w.LGraphCanvas && w.LGraphCanvas.prototype && w.document && w.document.body); }
    catch (_) { return false; }
  }
  ...
  function scanFrames(doc, host, depth) {
    ...
    var frames;
    try { frames = doc.querySelectorAll("iframe, frame"); } catch (_) { return null; }
    for (var i = 0; i < frames.length; i++) {
      var f = frames[i], w = null, d = null;
      try { w = f.contentWindow; d = f.contentDocument || (w && w.document); } catch (_) { continue; }
  ...
  function resolveEngineTarget() {
    ...
    engineTarget = null;
    if (frameHasLiveCanvas(window)) {          // we ARE the ComfyUI document
  ...
  function getLgc(t) {
    if (!t) return null;
    var w = t.win, cv = null;
    try { cv = w.app && w.app.canvas; } catch (_) {}
    if (!cv) { try { cv = w.LGraphCanvas && w.LGraphCanvas.active_canvas; } catch (_) {} }
    if (!cv) {
      try {
        var g = (w.app && w.app.graph) || w.graph;
        if (g && g.list_of_graphcanvas && g.list_of_graphcanvas.length) cv = g.list_of_graphcanvas[0];
      } catch (_) {}
    }
    try { if (cv && cv.ds && cv.canvas) return cv; } catch (_) {}
    return null;
  }
  function getGraph(t) {
    var cv = getLgc(t);
    try { if (cv && cv.graph) return cv.graph; } catch (_) {}
    try { return (t.win.app && t.win.app.graph) || t.win.graph || null; } catch (_) { return null; }
  }
  function getApp(t) {
    try { return (t && t.win && (t.win.app || (t.win.comfyAPI && t.win.comfyAPI.app && t.win.comfyAPI.app.app))) || null; }
    catch (_) { return null; }
  }
  ...
  function linkModeName(v) {
    for (var i = 0; i < LINK_MODES.length; i++) if (LINK_MODES[i].v === v) return LINK_MODES[i].name;
    return "Spline";
  }
  function applyLinkMode(v) {
    lsSet(LINKMODE_KEY, String(v));
    var t = engineTarget, cv = getLgc(t), app = getApp(t);
    // Prefer the frontend's own setting so RH's UI stays in sync.
    try { if (app && app.ui && app.ui.settings && app.ui.settings.setSettingValue) app.ui.settings.setSettingValue("Comfy.LinkRenderMode", v); } catch (_) {}
    try { if (app && app.extensionManager && app.extensionManager.setting && app.extensionManager.setting.set) app.extensionManager.setting.set("Comfy.LinkRenderMode", v); } catch (_) {}
    try {
      if (cv) {
  ...
  function cycleLinkMode() {
    var cur = currentLinkMode(), idx = 0;
    for (var i = 0; i < LINK_MODES.length; i++) if (LINK_MODES[i].v === cur) idx = i;
    applyLinkMode(LINK_MODES[(idx + 1) % LINK_MODES.length].v);
  }

  function redraw(cv) { try { cv.setDirty(true, true); } catch (_) {} }

  function enforceInstanceSettings(cv) {
    try {
      if (config.darkerGrid && typeof cv.clear_background_color === "string" &&
      ...
      if (config.enhancedWires) {
        ...
        if (cv.links_render_mode !== undefined && cv.links_render_mode !== want) cv.links_render_mode = want;
        if (cv.link_render_mode !== undefined && cv.link_render_mode !== want) cv.link_render_mode = want;
        if (cv.connections_width !== undefined && cv.connections_width < WIRE_WIDTH) {
          cv.connections_width = WIRE_WIDTH; redraw(cv);
        }
      ...
      if (config.fpsOptimization) {
        ...
        if (!config.enhancedWires && cv.render_connections_border) cv.render_connections_border = false;
      }
    } catch (_) {}
  }
  function injectCanvasCSS(t) {
    ...
    try {
      ...
      st.textContent =
        "body,html{background-color:" + CANVAS_BG + " !important;}" +
  ...
  function attachEngine(t) {
    var win = t.win, P = null;
    try { P = win.LGraphCanvas && win.LGraphCanvas.prototype; } catch (_) {}
    if (!P) return;
    try {
      if (win.__RH_ENGINE_OWNER__ && win.__RH_ENGINE_OWNER__ !== window) return;
      ...
      P.getNodeMenuOptions = function (node) {
        ...
        try {
          ...
          var already = options.some(function (o) {
            return o && o.content && (String(o.content).indexOf("Auto-Cancel") !== -1 ||
          ...
          var head = [];
          head.push({
            content: isStopId(node && node.id) ? "🛑 Remove Stop Breakpoint" : "🛑 Set Auto-Cancel Breakpoint",
            callback: function () {
              var on = toggleStopId(node.id);
              rhToast(on ? "🛑 Breakpoint SET on node " + node.id : "Breakpoint removed from node " + node.id);
              redraw(self);
            }
          });
          if (outputsOf(node, win).length) {
            head.push({
              content: "💾 Save output",
              callback: function () { saveOutputs(outputsOf(node, win)); }
            });
          }
          head.push(null);                      // separator
          options.unshift.apply(options, head); // top of the menu
        } catch (e) { warn("menu override", e); }
        return options;
      };
    ...
    if (api && win.__rh_hooked_api !== api) {
      ...
      var endRun = function (label) {
        ...
        win.__rh_last_done_str = win.__rh_run_start
          ? label + " in " + fmtDur(Date.now() - win.__rh_run_start) : label;
        log("run ended:", label);
      };
      ...
      try {
        api.addEventListener("executing", function (e) {
          try {
            var d = e && e.detail;
            var raw = (d && typeof d === "object")
              ? (d.node != null ? d.node : (d.display_node != null ? d.display_node : null))
              : d;
            if (raw == null) { win.__rh_running = null; endRun("✅ Done"); return; }
            if (!win.__rh_run_active) {           // recover a run already going
            ...
            if (config.autoCancelBreakpoints && isStopId(raw)) triggerRHInterrupt({ win: win, doc: t.doc });
          } catch (err) { warn("executing handler", err); }
        });
        ...
        api.addEventListener("progress", function (e) {
          try {
            var d = e && e.detail;
            if (!d) return;
            win.__rh_prog = { v: Number(d.value) || 0, max: Number(d.max) || 0 };
          } catch (_) {}
        });
        ...
        api.addEventListener("execution_error", function () { endRun("❌ Error"); });
        api.addEventListener("execution_interrupted", function () { endRun("⏸ Cancelled"); });
        api.addEventListener("execution_success", function () { endRun("✅ Done"); });
      } catch (e) { warn("api hook failed", e); }
    }
  }
  /* ======================= 10. overlay canvas ==========================
  ...
  function viewportFactor(lgc, win) {
    try {
      var el = lgc.canvas, base = el.getBoundingClientRect();
  ...
  function nodeScreenRect(lgc, node) {
    try {
      ...
      var cx, cy, conv = null;
      if (typeof lgc.convertOffsetToCanvas === "function") conv = lgc.convertOffsetToCanvas.bind(lgc);
      else if (lgc.ds && typeof lgc.ds.convertOffsetToCanvas === "function") conv = lgc.ds.convertOffsetToCanvas.bind(lgc.ds);
      if (conv) { var p = conv([node.pos[0], node.pos[1]]); cx = p[0]; cy = p[1]; }
      else {
        var off = lgc.ds.offset || [0, 0];
        var oxx = Array.isArray(off) ? off[0] : (off.x || 0);
        var oyy = Array.isArray(off) ? off[1] : (off.y || 0);
        cx = (node.pos[0] + oxx) * s; cy = (node.pos[1] + oyy) * s;
      }
      var titleH = 30;
      try {
        var LG = engineTarget && engineTarget.win && engineTarget.win.LiteGraph;
      ...
      return { x: base.left + cx * f, y: base.top + (cy - titleH * s) * f,
               w: gw * s * f, h: (gh + titleH) * s * f };
    } catch (_) { return null; }
  }
  function onScreen(r) {
    return !!(r && r.x < ov.w && r.y < ov.h && r.x + r.w > 0 && r.y + r.h > 0);
  }
  function drawBreakpointBox(ctx, r) {
    ...
    if (solid) { ctx.lineWidth = 5; ctx.setLineDash([]); }
    else { ctx.lineWidth = 3.5; ctx.setLineDash([9, 5]); }
  ...
  function drawRunning(ctx, r, phase, prog) {
    ...
    ctx.save();
    ctx.lineWidth = 6;
    ctx.strokeStyle = "rgba(0,255,102," + (0.10 + 0.14 * pulse).toFixed(3) + ")";
    ctx.strokeRect(x - 3, y - 3, w + 6, h + 6);
    ctx.lineWidth = 3.5;
    ctx.strokeStyle = "rgba(0,255,102," + (0.55 + 0.45 * pulse).toFixed(3) + ")";
    ctx.lineCap = "round";
    ctx.beginPath();
  ...
  function overlayLoop() {
    ...
    try {
      ...
      try { if (t.win.__RH_ENGINE_OWNER__ && t.win.__RH_ENGINE_OWNER__ !== window) return; } catch (_) { return; }
      var win = t.win;
      var showBp = config.autoCancelBreakpoints && stopNodeIds.size > 0;
      var running = config.vectorNodeIndicator && win.__rh_running;
      if (!showBp && !running) {
        if (ov.dirty) {                    // one final clear, then go idle
      ...
      var lgc = getLgc(t), graph = lgc && lgc.graph;
      ...
      if (running) {
        ...
        if (rn) {
          ...
          if (onScreen(rr)) drawRunning(ctx, rr, performance.now(), win.__rh_prog);
        }
      }
    } catch (_) { /* overlay errors must never reach the page */ }
  }
  requestAnimationFrame(overlayLoop);
  /* ======================= 11. title status + close guard ============== */
  function engineWin() { return engineTarget ? engineTarget.win : window; }
  ...
  function titleTick() {
    try {
      ...
      if (!base) {
        base = (document.title || "RunningHub").replace(/\s*[·▶✅❌⏸][\s\S]*$/, "").trim() || "RunningHub";
        w.__rh_title_base = base;
      }
      if (w.__rh_run_active && w.__rh_run_start) {
        document.title = base + " · ▶ " + fmtDur(Date.now() - w.__rh_run_start) + " · Running";
      } else if (w.__rh_last_done_str && w.__rh_run_end && Date.now() - w.__rh_run_end < 20000) {
  ...
  cleanups.push(function () { clearInterval(titleTimer); });

  window.addEventListener("beforeunload", function (e) {
    try {
      if (destroyed || !config.closeGuard || !IS_TOP || !IS_WORKFLOW_WINDOW) return;
      var w = engineWin();
      if (w && (w.__rh_run_active || w.__rh_running)) { e.preventDefault(); e.returnValue = ""; }
    } catch (_) {}
  });
  ...
  function ensureHUD() {
    ...
    hud.id = "rh-timer-hud";
    hud.innerHTML = '<span style="font-size:14px;">⏱️</span><span id="rh-hud-time">0s</span>';
  ...
  function updateHUD() {
    ...
    try {
      ...
      if (w && w.__rh_run_active) {
        ...
        if (!row) {
          if (++noTaskRowTicks >= 3 && typeof w.__rh_endRun === "function") w.__rh_endRun("✅ Done");
        } else noTaskRowTicks = 0;
      } else noTaskRowTicks = 0;

      if (w && w.__rh_run_active) {
        var own = w.__rh_run_start ? fmtDur(Date.now() - w.__rh_run_start) : "…";
        var label = own;
        var st = d.querySelector(".rh-task-item [class*='task-status-']") ||
      ...
      if (w && w.__rh_last_done_str && w.__rh_run_end && Date.now() - w.__rh_run_end < 8000) {
        if (disp && disp.textContent !== w.__rh_last_done_str) disp.textContent = w.__rh_last_done_str;
        if (hud.style.display !== "flex") hud.style.display = "flex";
        return;
      }
  ...
  function outputsOf(node, win) {
    ...
    try {
      if (!list.length && node && node.imgs && node.imgs.length) {
        for (var j = 0; j < node.imgs.length; j++) {
          if (node.imgs[j] && node.imgs[j].src) list.push({ url: node.imgs[j].src });
        }
      }
    } catch (_) {}
    return list.map(function (o) {
      ...
      var q = o.q;
      var path = "/view?filename=" + encodeURIComponent(q.filename || "") +
                 "&type=" + encodeURIComponent(q.type || "output") +
                 "&subfolder=" + encodeURIComponent(q.subfolder || "");
      var u = path;
      try { if (win.api && typeof win.api.apiURL === "function") u = win.api.apiURL(path); } catch (_) {}
      return { url: absUrl(u, win), name: (q.filename || nameFromUrl(u)).split("/").pop() };
    });
  }
  ...
  function saveOne(item, quiet) {
    ...
    if (inv) {
      try {
        inv("download_file", { params: { url: item.url, filename: name, language: "en" } });
        if (!quiet) rhToast("⬇️ Saving " + name + " → Downloads");
        return true;
      } catch (e) { warn("tauri download failed", e); }
    }
    try {                                  // a real <a download> in the DOM
      var d = topDoc();
      var a = d.createElement("a");
      a.href = item.url; a.download = name;
      ...
      return true;
    } catch (e) { warn("anchor download failed", e); }
    try { window.open(item.url, "_blank"); rhToast("↗️ Opened " + name + " — save it from there", 4000); return true; }
    catch (e) { rhToast("❌ Could not save " + name); return false; }
  }
  function saveOutputs(items) {
    if (!items || !items.length) { rhToast("No outputs on this node yet"); return; }
    if (items.length === 1) { saveOne(items[0]); return; }
    rhToast("⬇️ Saving " + items.length + " files → Downloads");
    items.forEach(function (it, i) { setTimeout(function () { saveOne(it, true); }, i * 400); });
  }
  function saveAllRunOutputs() {
    var t = engineTarget, g = getGraph(t), app = getApp(t);
    if (!t || !g || !app || !app.nodeOutputs) { rhToast("No outputs to save yet"); return; }
    var all = [];
    try {
  ...
  var TRIM_PATTERNS = [
    /favicon/i, /\.locking$/i, /snaptogrid/i, /guide/i, /tutorial/i, /imagefeed/i,
    /image[_-]?feed/i, /analytics/i, /telemetry/i, /\.tips?$/i, /wechat/i, /qrcode/i,
    /banner/i, /promo/i, /announce/i, /changelog/i, /whatsnew/i
  ];
  ...
  function extSettingApi(app) {
    try {
      if (app && app.extensionManager && app.extensionManager.setting &&
        ...
        return {
          get: function () { return app.extensionManager.setting.get(EXT_SETTING); },
          set: function (v) { return app.extensionManager.setting.set(EXT_SETTING, v); }
        };
      }
      if (app && app.ui && app.ui.settings &&
        ...
        return {
          get: function () { return app.ui.settings.getSettingValue(EXT_SETTING); },
          set: function (v) { return app.ui.settings.setSettingValue(EXT_SETTING, v); }
        };
      }
    } catch (_) {}
    return null;
  ...
  function extensionNames(app) {
    var names = [];
    try {
      var ex = app && (app.extensions || (app.extensionManager && app.extensionManager.extensions));
      if (ex && ex.length) {
  ...
  function applyExtensionTrim(on) {
    var t = engineTarget, app = getApp(t);
    var api = extSettingApi(app);
    if (!api) { rhToast("⚠️ This ComfyUI build has no extension setting — trim skipped", 5000); return false; }
    var cur = [];
    try { var v = api.get(); if (Array.isArray(v)) cur = v.slice(); } catch (_) {}
    if (on) {
      if (lsGet(EXT_PREV_KEY) == null) lsSet(EXT_PREV_KEY, JSON.stringify(cur));
      var names = extensionNames(app);
      var add = names.filter(function (n) {
        return cur.indexOf(n) === -1 && TRIM_PATTERNS.some(function (rx) { return rx.test(n); });
      });
      log("extensions seen:", names);
      if (!add.length) { rhToast("Nothing safe to trim (" + names.length + " extensions seen)", 4000); return false; }
      try { api.set(cur.concat(add)); } catch (e) { rhToast("⚠️ Could not write the setting"); return false; }
      rhToast("✂️ Trimmed " + add.length + " cosmetic extensions — reload to apply", 6000);
      return true;
    }
    var prev = safeParse(EXT_PREV_KEY, null);
    try { api.set(Array.isArray(prev) ? prev : []); } catch (_) {}
    try { localStorage.removeItem(EXT_PREV_KEY); } catch (_) {}
  ...
  var PROTECTED_RX = /^(RH_MAIN_SCRIPT_OVERRIDE|RH_LAST_GOOD_SCRIPT|RH_QOL_CONFIG|RH_BOOKMARKS|RH_FLOATING_POS|RH_LINK_MODE|RH_EXT_PREV_DISABLED|RH_COMFY_BUNDLE|RH_CUSTOM_SCRIPTS_LIST)$|token|auth|session|user|login|jwt/i;
  function storageReport() {
    var groups = { comfy: 0, rh: 0, ours: 0, other: 0 }, counts = { comfy: 0, rh: 0, ours: 0, other: 0 };
    try {
      for (var i = 0; i < localStorage.length; i++) {
        var k = localStorage.key(i);
        var v = localStorage.getItem(k) || "";
        var size = (k.length + v.length) * 2;         // UTF-16 bytes
        var g = /^Comfy\.|^workflow|^litegraph|^Comfy_/i.test(k) ? "comfy"
              : /^RH_/.test(k) ? "ours"
              : /runninghub|rh[-_]/i.test(k) ? "rh" : "other";
        groups[g] += size; counts[g]++;
      }
    } catch (_) {}
  ...
  function freePreviewMemory() {
    var t = engineTarget, app = getApp(t);
    var freed = 0;
    try { freed = app && app.nodeOutputs ? Object.keys(app.nodeOutputs).length : 0; } catch (_) {}
    try {
      if (app && typeof app.clean === "function") { app.clean(); rhToast("🧹 Freed preview memory (" + freed + " nodes)"); return; }
      if (app) { app.nodeOutputs = {}; app.nodePreviewImages = {}; rhToast("🧹 Cleared cached outputs"); return; }
      rhToast("Engine not attached — nothing to free");
    } catch (e) { rhToast("⚠️ Could not free memory: " + e.message); }
  }
  function clearComfyCacheKeys() {
    ...
    try {
      ...
      for (var i = 0; i < localStorage.length; i++) {
        ...
        if (/^Comfy\.PreviousWorkflow|^workflow$|^litegraph|clipboard|_cache$|Comfy\.NodeLibrary\.Bookmarks\.cache/i.test(k)) kill.push(k);
      }
    ...
    rhToast(removed ? "🧹 Removed " + removed + " cache keys (" + fmtBytes(freed) + ")" : "Nothing to remove");
  }
  function openCachePanel() {
    ...
    ovl.style.cssText = "position:fixed;inset:0;z-index:1000002;background:rgba(0,0,0,.6);" +
    ...
    ovl.innerHTML =
      '<div style="width:min(560px,94vw);background:#16161a;border:1px solid #333;border-radius:8px;padding:16px;color:#ddd;box-shadow:0 12px 40px rgba(0,0,0,.8);">' +
        ...
        '<div style="font-size:11px;line-height:1.9;color:#bbb;margin-bottom:12px;">' +
          "ComfyUI keys: <b>" + fmtBytes(rep.groups.comfy) + "</b> (" + rep.counts.comfy + ")<br>" +
          "RunningHub keys: <b>" + fmtBytes(rep.groups.rh) + "</b> (" + rep.counts.rh + ")<br>" +
          "Our keys (protected): <b>" + fmtBytes(rep.groups.ours) + "</b> (" + rep.counts.ours + ")<br>" +
          "Other: <b>" + fmtBytes(rep.groups.other) + "</b> (" + rep.counts.other + ")" +
        '</div>' +
    ...
    var close = function () { if (ovl.parentNode) ovl.parentNode.removeChild(ovl); };
    ovl.querySelector("#rh-cache-close").onclick = close;
    ovl.querySelector("#rh-cache-mem").onclick = function () { freePreviewMemory(); close(); };
    ovl.querySelector("#rh-cache-keys").onclick = function () { clearComfyCacheKeys(); close(); };
  }

  /* ======================= 16. bookmarks + nav (base window) =========== */
  function getBookmarks() { var b = safeParse(BOOKMARK_KEY, []); return Array.isArray(b) ? b : []; }
  function saveBookmarks(b) { lsSet(BOOKMARK_KEY, JSON.stringify(b.slice(0, 60))); }
  function openBookmarkPanel() {
    ...
    p.id = "rh-bm-panel";
    p.style.cssText = "position:fixed;left:14px;bottom:52px;z-index:1000001;width:300px;max-height:50vh;" +
    ...
    var rows = list.map(function (b, i) {
      return '<div style="display:flex;gap:6px;align-items:center;margin:4px 0;">' +
        '<span data-go="' + i + '" style="flex:1;cursor:pointer;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;" title="' + esc(b.url) + '">' + esc(b.title || b.url) + '</span>' +
        '<span data-ren="' + i + '" style="cursor:pointer;color:#88c;">✎</span>' +
        '<span data-del="' + i + '" style="cursor:pointer;color:#f66;">✕</span></div>';
    }).join("") || '<div style="color:#666;font-style:italic;">No bookmarks yet</div>';
    p.innerHTML = '<div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:8px;">' +
      '<b>⭐ Bookmarks</b><button id="rh-bm-add" style="padding:3px 8px;background:#00AA55;border:0;color:#fff;border-radius:4px;cursor:pointer;font-size:10px;">+ Add this page</button></div>' + rows;
    ...
    p.querySelector("#rh-bm-add").onclick = function () {
      ...
      } catch (e) { deflt = "RunningHub"; }
      openPasteModal("Name this bookmark", "type a name…", function (name) {
    ...
    p.addEventListener("click", function (e) {
      ...
      if (go != null) { var b1 = getBookmarks()[Number(go)]; if (b1) location.href = b1.url; }
      else if (ren != null) {
        var idx = Number(ren), br = getBookmarks()[idx];
        if (br) openPasteModal("Rename bookmark", "type a name…", function (name) {
          var all = getBookmarks();
          all[idx] = { title: (name || "").slice(0, 60) || br.title, url: br.url };
          saveBookmarks(all);
          var pnl = topDoc().getElementById("rh-bm-panel");
          if (pnl) pnl.parentNode.removeChild(pnl);
          openBookmarkPanel();
      ...
      else if (del != null) { var b2 = getBookmarks(); b2.splice(Number(del), 1); saveBookmarks(b2); p.parentNode.removeChild(p); openBookmarkPanel(); }
    });
  }
  function createNavBar() {
    ...
    bar.id = "rh-navbar";
    bar.style.cssText = "position:fixed;left:12px;bottom:12px;z-index:999999;display:flex;gap:4px;" +
      "opacity:.45;transition:opacity .15s;font-family:monospace;";
    bar.onmouseenter = function () { bar.style.opacity = "1"; };
    bar.onmouseleave = function () { bar.style.opacity = ".45"; };
    var mk = function (txt, title) {
      var b = document.createElement("div");
      b.textContent = txt;
      b.title = title;
      b.style.cssText = "width:24px;height:24px;border-radius:6px;background:rgba(22,22,26,.9);" +
        "border:1px solid #3a3a44;color:#ccc;display:flex;align-items:center;justify-content:center;" +
        "cursor:pointer;font-size:13px;user-select:none;";
      return b;
    };
    var back = mk("‹", "Back"), fwd = mk("›", "Forward"), bm = mk("★", "Bookmarks");
    back.onclick = function () { try { history.back(); } catch (_) {} };
    fwd.onclick = function () { try { history.forward(); } catch (_) {} };
    bm.onclick = function () { openBookmarkPanel(); };
    bar.appendChild(back); bar.appendChild(fwd); bar.appendChild(bm);
  ...
  function createFloatingMenu() {
    ...
    var pos = {
      x: Math.max(8, Math.min(Number.isFinite(raw.x) ? raw.x : 20, window.innerWidth - 46)),
      y: Math.max(8, Math.min(Number.isFinite(raw.y) ? raw.y : 20, window.innerHeight - 46))
    };

    var toggles = Object.keys(DEFAULT_CONFIG).map(function (k) {
      return '<label style="display:flex;justify-content:space-between;align-items:center;margin:5px 0;font-size:11px;cursor:pointer;gap:8px;">' +
        '<span>' + esc(CORE_LABELS[k] || k) + '</span>' +
        '<input type="checkbox" data-core-key="' + esc(k) + '"' + (config[k] ? " checked" : "") + ' style="cursor:pointer;"></label>';
    }).join("");
    ...
    box.innerHTML =
      ...
      '<div id="rh-settings-panel" style="display:none;">' +
        ...
        '<button id="rh-linkmode-btn" style="width:100%;margin-bottom:6px;padding:6px;background:#2b2b36;border:1px solid #555;color:#ddd;border-radius:4px;cursor:pointer;font-size:11px;">🔗 Link style: ' + linkModeName(currentLinkMode()) + '</button>' +
        ...
        '<div id="rh-core-toggles" style="margin-bottom:8px;">' + toggles + '</div>' +
    ...
    srcEl.textContent = "v" + VERSION + " · live copy: " +
      (src === "cache" ? "LAST-GOOD CACHE" : src === "override" ? "OVERRIDE" : "BUNDLED") +
    ...
    function placePanel() {
      ...
      if (r.top < 340) { panel.style.bottom = "auto"; panel.style.top = "46px"; }
      else { panel.style.top = "auto"; panel.style.bottom = "46px"; }
      if (r.left > window.innerWidth - 330) { panel.style.left = "auto"; panel.style.right = "0"; }
      else { panel.style.right = "auto"; panel.style.left = "0"; }
    }
    var dragging = false, sx = 0, sy = 0, ix = 0, iy = 0, moved = 0;
    function onMove(e) {
      if (!dragging) return;
      var dx = e.clientX - sx, dy = e.clientY - sy;
      moved = Math.hypot(dx, dy);
      box.style.left = Math.max(8, Math.min(ix + dx, window.innerWidth - 46)) + "px";
      box.style.top = Math.max(8, Math.min(iy + dy, window.innerHeight - 46)) + "px";
    }
    function endDrag() {
      if (!dragging) return;
      dragging = false;
      ...
      var r = box.getBoundingClientRect();
      lsSet(POS_KEY, JSON.stringify({ x: Math.round(r.left), y: Math.round(r.top) }));
      placePanel();
    }
    toggleBtn.addEventListener("pointerdown", function (e) {
      if (e.button !== 0) return;
      dragging = true; sx = e.clientX; sy = e.clientY; moved = 0;
      var r = box.getBoundingClientRect(); ix = r.left; iy = r.top;
    ...
    function renderScripts() {
      if (!customScripts.length) {
        listEl.innerHTML = '<div style="font-size:10px;color:#666;font-style:italic;">None added.</div>';
        return;
      }
      listEl.innerHTML = customScripts.map(function (s, i) {
        return '<div style="display:flex;justify-content:space-between;align-items:center;margin:4px 0;background:#101014;padding:3px 6px;border-radius:4px;font-size:11px;">' +
          '<label style="cursor:pointer;display:flex;align-items:center;gap:6px;">' +
          '<input type="checkbox" data-script-idx="' + i + '"' + (s.enabled ? " checked" : "") + '>' +
          '<span style="max-width:180px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;">' + esc(s.name) + '</span></label>' +
          '<span data-del-idx="' + i + '" style="cursor:pointer;color:#f66;font-size:12px;">🗑️</span></div>';
      }).join("");
    }
    renderScripts();
    ...
    box.querySelector("#rh-close-btn").onclick = function () { panel.style.display = "none"; };
    box.querySelector("#rh-reload-btn").onclick = function () { location.reload(); };
    box.querySelector("#rh-edit-script-btn").onclick = function () { panel.style.display = "none"; openScriptEditor(); };
    box.querySelector("#rh-import-cookies-btn").onclick = function () { pasteAndImportCookies(); };
    box.querySelector("#rh-cache-btn").onclick = function () { panel.style.display = "none"; openCachePanel(); };
    box.querySelector("#rh-saveall-btn").onclick = function () { panel.style.display = "none"; saveAllRunOutputs(); };
    box.querySelector("#rh-linkmode-btn").onclick = function (e) {
      cycleLinkMode();
      e.target.textContent = "🔗 Link style: " + linkModeName(currentLinkMode());
    };
    box.querySelector("#rh-add-script-btn").onclick = function () {
      var nameEl = box.querySelector("#rh-new-script-name"), codeEl = box.querySelector("#rh-new-script-code");
      var code = codeEl.value.trim();
      if (!code) { rhToast("⚠️ Enter some code first"); return; }
      try { new Function(code); } catch (err) { rhToast("❌ Syntax error: " + err.message); return; }
      customScripts.push({ id: Date.now(), name: nameEl.value.trim() || ("Script #" + (customScripts.length + 1)), code: code, enabled: true });
    ...
    box.querySelector("#rh-save-apply-btn").onclick = function () {
      try {
        ...
        saveConfig();
        box.querySelectorAll("input[data-script-idx]").forEach(function (inp) {
          var i = Number(inp.getAttribute("data-script-idx"));
          if (customScripts[i]) customScripts[i].enabled = inp.checked;
        });
        saveCustomScripts();
        if (!!config.trimExtensions !== trimWas) applyExtensionTrim(!!config.trimExtensions);
        setTimeout(function () { location.reload(); }, 600);
      } catch (e) { rhToast("❌ Save failed: " + e.message); }
    };
  }
  ...
  function openScriptEditor() {
    ...
    var overrideSrc = lsGet(SCRIPT_OVERRIDE_KEY), cacheSrc = lsGet(SCRIPT_CACHE_KEY);
    var src = window.__RH_LIVE_SOURCE__;
    var label = (src === "override" || overrideSrc != null) ? "OVERRIDE (your edited copy is live)"
              : src === "cache" ? "LAST-GOOD CACHE (saving creates an override)"
              : "BUNDLED v" + VERSION + " (compiled in — saving creates an override)";
    var prefill = overrideSrc != null ? overrideSrc : (cacheSrc != null ? cacheSrc :
    ...
    o.innerHTML =
      '<div style="width:min(920px,94vw);height:min(80vh,720px);background:#16161a;border:1px solid #333;border-radius:8px;display:flex;flex-direction:column;overflow:hidden;font-family:monospace;color:#ddd;box-shadow:0 12px 40px rgba(0,0,0,.8);">' +
        '<div style="padding:10px 14px;border-bottom:1px solid #333;display:flex;justify-content:space-between;gap:10px;flex-wrap:wrap;">' +
          ...
          '<div style="font-size:10px;color:#00FF66;">LIVE: ' + esc(label) + '</div></div>' +
    ...
    o.querySelector("#rh-edit-remove").onclick = function () {
      try { localStorage.removeItem(SCRIPT_OVERRIDE_KEY); } catch (_) {}
      location.reload();
    };
    o.querySelector("#rh-edit-save").onclick = function () {
      var code = ta.value;
      if (!code || !code.trim()) { rhToast("⚠️ Refusing to save an empty script"); return; }
      try { new Function(code); } catch (err) { rhToast("❌ Syntax error: " + err.message, 6000); return; }
      if (code.indexOf("__RH_SCRIPT_BOOT_OK__") === -1) {
        rhToast("❌ Not a full custom_r.js (no boot handshake) — not saved", 7000); return;
      }
      if (!lsSet(SCRIPT_OVERRIDE_KEY, code)) { rhToast("❌ Save failed (storage blocked)"); return; }
  ...
  /* ======================= 20. poller ================================= */
  var engineReady = false, engineMisses = 0, warnedNoEngine = false;
  function pollTick() {
    if (destroyed) return;
    try {
      if (SHOULD_LOCK_TITLE) {
        var tEl = document.querySelector("title");
        if (tEl && tEl.textContent !== "RunningHub") tEl.textContent = "RunningHub";
      }
    } catch (_) {}

    try {
      if (SHOW_UI && document.body) {
        if (!document.getElementById("rh-floating-root")) createFloatingMenu();
        createNavBar();
        ensureHUD();
      }
    } catch (e) { warn("ui tick", e); }
    ...
    try {
      ...
      if (t) {
        ...
        if (cv) {
          ...
          if (!engineReady) {
            engineReady = true;
            log("engine ready", t.host ? "iframe" : "self", cv.ds && cv.ds.scale);
          }
        }
      } else if (IS_WORKFLOW_WINDOW && !engineReady) {
        engineMisses++;
        if (engineMisses === 20 && !warnedNoEngine) {          // ~12s
          warnedNoEngine = true;
          rhToast("⚠️ ComfyUI engine not found — canvas features are off (frames:" +
                  document.querySelectorAll("iframe").length + ")", 8000);
        }
      }
    } catch (e) { warn("engine tick", e); }

    try { updateHUD(); } catch (_) {}
  }
  var pollTimer = setInterval(pollTick, 600);
  cleanups.push(function () { clearInterval(pollTimer); });
  pollTick();
  prefetchComfy();
  ...
  window.__RH_EXT_TEARDOWN__ = function () {
    ...
    ["rh-floating-root", "rh-floating-style", "rh-timer-hud", "rh-toast-host", "rh-navbar",
     "rh-bm-panel", "rh-modal-overlay", "rh-edit-overlay", "rh-cache-overlay",
     "rh-firstrun-overlay"].forEach(function (id) {
  ...
  window.__RH_DEBUG__ = {
    ...
    engine: function () { return engineTarget; },
    lgc: function () { return getLgc(engineTarget); },
    app: function () { return getApp(engineTarget); },
    outputs: function () { return saveAllRunOutputs; },
    cancelDom: function () { return cancelDiagnostics(); },
    cancel: function () { return triggerRHInterrupt(engineTarget || { win: window, doc: document }); },
    linkMode: cycleLinkMode,
    teardown: function () { return window.__RH_EXT_TEARDOWN__(); }
  };
  ...
  log("v" + VERSION + " initialized", {
    top: IS_TOP, workflow: IS_WORKFLOW_WINDOW, comfyDoc: IS_COMFY_DOC,
