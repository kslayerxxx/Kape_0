...
(function () {
  ...
  function install(src, label) {
    try {
      ...
      if (window.__RH_SCRIPT_BOOT_OK__ !== true) {
        if (window.console)
          console.error(
            "[RH bootstrap] " + label + " copy never signalled boot",
          );
        return false;
      }
      ...
      if (window.console)
        console.error("[RH bootstrap] " + label + " copy threw:", err);
      return false;
    }
  }
  ...
  var installed = null;
  if (override && override.trim() && install(override, "override"))
    installed = override;
  if (!installed && cache && cache.trim() && install(cache, "cache"))
    installed = cache;
