...
(function () {
  ...
  try {
    ...
    function resolveEngineTarget() {
      ...
      if (comfyIframe) {
        try {
          ...
          /* cross-origin — retry next tick */
        }
        return null;
      }
      if (looksLikeComfyWindow() && document.body)
    ...
    function attachEngine(t) {
      ...
      if (win.api && win.__rh_hooked_api !== win.api) {
        ...
        try {
          ...
          try {
            api.addEventListener("execution_start", clearRunning);
          } catch (_) {}
          try {
            api.addEventListener("execution_success", clearRunning);
          } catch (_) {}
          try {
            api.addEventListener("execution_error", clearRunning);
          } catch (_) {}
          try {
            api.addEventListener("execution_interrupted", clearRunning);
          } catch (_) {}
          try {
    ...
    console.info(TAG, "v2.3 initialized", {
      IS_POPUP,
      IN_IFRAME,
    ...
    console.error(
      TAG,
      "FATAL init error — will retry on next injection.",
      fatalErr,
    );
  }
})();
