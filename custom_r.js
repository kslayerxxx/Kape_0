(function () {
  ...
  try {
    const origTitleDesc = Object.getOwnPropertyDescriptor(document, "title") ||
                          Object.getOwnPropertyDescriptor(Document.prototype, "title");
    if (origTitleDesc && origTitleDesc.configurable) {
      cleanups.push(() => {
        try { delete document.title; Object.defineProperty(document, "title", origTitleDesc); } catch (_) {}
      });
    }
    ...
    Object.defineProperty(document, "title", {
      ...
      set() { /* SPA router writes are swallowed */ },
    });
  } catch (e) {
    console.warn(TAG, "title override failed (poller fallback still active)", e);
  }
  ...
  function patchWindowOpen(w) {
    ...
    try {
      w.open = patched;
      cleanups.push(() => { try { w.open = origOpen; w.__RH_OPEN_PATCHED__ = false; } catch (_) {} });
    } catch (e) {
  ...
  function extractExecId(detail) {
    ...
    if (typeof detail === "object") {
      raw = detail.node != null ? detail.node : (detail.display_node != null ? detail.display_node : null);
    }
  ...
  function injectCanvasEngine() {
    ...
    if (iWin.__RH_IFRAME_ENGINE_OWNER__ && iWin.__RH_IFRAME_ENGINE_OWNER__ !== window) return;
    iWin.__RH_IFRAME_ENGINE_OWNER__ = window;

    // Also constrain workflow popups launched FROM the iframe (same-origin).
    try { patchWindowOpen(iWin); } catch (_) {}
    ...
    if (config.darkerGrid && !iDoc.getElementById("rh-canvas-perf-style")) {
      ...
      } catch (e) { console.warn(TAG, "iframe style injection failed", e); }
    }
        ...
        P.drawNode = function (node, ctx) {
          ...
          try {
            ...
            if (config.vectorNodeIndicator) {
              isRunning = !!(node.is_executing || node.running || this.node_executing === node);
            }
    ...
    if (config.enhancedWires && iWin.LiteGraph && !iWin.__rh_link_mode_set) {
      ...
      } catch (e) { console.warn(TAG, "LINK_RENDER_MODE failed", e); }
    }
    ...
    if (iWin.api && iWin.__rh_hooked_api !== iWin.api) {
      ...
      try {
        api.addEventListener("executing", (e) => {
          try {
            ...
            const rawId = (d && typeof d === "object")
              ...
              : d;
            if (rawId == null) { lastExecutedNodeId = null; return; }

            const idStr = String(rawId);
            if (idStr === lastExecutedNodeId) return;
            lastExecutedNodeId = idStr;

            if (config.autoCenterRunningNode && iWin.app && iWin.app.graph && iWin.app.canvas &&
        ...
        api.addEventListener("execution_start", () => { lastExecutedNodeId = null; });
      } catch (e) {
  ...
  function ensureTaskTimerHUD() {
    ...
    } catch (e) { /* body not ready — next tick */ }
  }
  ...
  function updateTaskTimerHUD() {
    ...
    } catch (e) { /* transient DOM churn — next tick */ }
  }
  ...
  function createFloatingMenu() {
    ...
    function adjustPanelPosition() {
      ...
      if (!panel) return;
      if (rect.top < 300) {
        panel.style.bottom = "auto"; panel.style.top = "46px";
      } else {
        panel.style.top = "auto"; panel.style.bottom = "46px";
      }
    ...
    function onPointerMove(e) {
      ...
      let nextX = initialLeft + dx, nextY = initialTop + dy;
    ...
    function renderCustomScriptList() {
      if (customScripts.length === 0) {
        userScriptList.innerHTML =
          `<div style="font-size: 10px; color: #666; font-style: italic;">No extra custom scripts added.</div>`;
        return;
      }
  ...
  function pollTick() {
    ...
    try {
      if (document.body && !document.getElementById("rh-floating-root")) createFloatingMenu();
    } catch (e) { console.warn(TAG, "floating menu tick failed", e); }

    try { injectCanvasEngine(); } catch (e) { console.warn(TAG, "canvas engine tick failed", e); }

    try { ensureTaskTimerHUD(); updateTaskTimerHUD(); } catch (e) { /* silent: cosmetic */ }
  }
  ...
  console.info(TAG, "v2.0 initialized — features:", Object.keys(config).filter(k => config[k]).join(", "));

  } catch (fatalErr) {
    window.__RH_EXT_INITIALIZED__ = false;
    console.error(TAG, "FATAL init error — will retry on next injection. Run __RH_EXT_TEARDOWN__() to clean partial state.", fatalErr);
  }
})();
